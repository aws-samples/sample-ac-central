...
```bash
# Start an A/B test with 10% treatment traffic
agentcore run ab-test \
  --control-weight 90 \
  --treatment-weight 10 \
  --control-config ./configs/v1-config.json \
  --treatment-config ./configs/v2-config.json
```

The equivalent API operations are `CreateABTest` and `GetABTest`. Both control and treatment variants are scored by online evaluators in parallel; the platform reports per-variant scores and statistical significance as traffic accumulates.
